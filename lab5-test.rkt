...
;; Test cases contributed by Josh Crain, Fall 2024
(check =
       (evaluate '(let ((a +) (b 2)) (a (let ((c *)) (c b x)) 3)) e1)
       23)
(check =
       (evaluate '(let ((a +) (b 2)) (a b b)) e1)
       4)
(check equal?
       (evaluate '(let ((m cons)) (m 1 (m 2 (m 3 nil)))) e1)
       '(1 2 3))
(check equal?
       (evaluate '(let ((n +)) (let ((m (cons (n 3 3) 4))) m)) e1)
       '(6 . 4))
